...
3. `S` キー、または「設定」でテーマを選ぶ（複数可）
   ...
   - シベリア：タイガと木造の村、バイカルの氷と冬の集落、木造建築の街、シベリア鉄道
   - 静かな部屋：窓辺の椅子、テーブルの上の一杯、カーテン越しの光。要素の少ない室内の一角
4. 気に入った一枚は `L` キーか「お気に入り」で登録。設定の「お気に入り」から JSON で書き出せます
5. 「見せかた」の「雨」を右に動かすと、雨の日の窓ガラス越しに眺める演出になります（小雨から豪雨まで。`R` キーで段階的に切り替え）。
   雨のときは雨音も重なります。「雨音」で音量を、「雨音の候補を聴く」で小雨・雨・豪雨それぞれの音を選べます
6. 「音楽」の「曲を選ぶ」で手元の曲を選ぶ（選んだ順に繰り返す）
   （右下の時計にはロンドンとニューヨークの時刻も出ます。「見せかた」で消せます）
