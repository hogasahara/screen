...
### 機能
- テーマ6つをチェックボックスで組み合わせ（複数可）。テーマごとに英語の検索語リスト `THEMES` を持つ。ブルーモーメント／ヨーロッパの村／プリンス・エドワード島／雪と灯り／田園と空／日本のレトロな街
- 切り替え間隔 8〜120秒
...
- 認定カテゴリ（秀逸・品質）は風景系には多いが、日本のレトロな街並みにはほとんど無い。そこは地名（銀山温泉、城崎、奈良井宿、妻籠、倉敷、尾道、伊根、高山）で通常検索の外れを減らしている
...
- Commonsの検索語の当たり具合。`THEMES` の語は調整前提。特に「日本のレトロな街」は当たり枚数しか見ていない
